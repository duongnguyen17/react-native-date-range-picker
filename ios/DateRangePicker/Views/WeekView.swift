...
final class WeekView: UIView {
    ...
    func makeWeekLabel(for symbol: String) -> UILabel {
      ...
      // Kiểm tra xem ngày đó có phải là cuối tuần (thứ 6 hoặc thứ 7) hay không
          let isWeekend = isWeekendDay(weekdaySymbol: symbol)
          // Đặt màu chữ cho cuối tuần thành màu đỏ, ngược lại là màu textColor đã cấu hình trong config
          label.textColor = isWeekend ? .red : self.config.textColor
  ...
  private func isWeekendDay(weekdaySymbol: String) -> Bool {
      let calendar = Calendar.current
      guard let weekdayIndex = calendar.shortStandaloneWeekdaySymbols.firstIndex(of: weekdaySymbol) else {
          return false
      }
