...
final class DayCell: JTACDayCell {
    ...
    public static func makeViewConfig(
        ...
        calendar: Calendar,
        weekendColor: UIColor = .red,
        dateLabelColor: UIColor = .label
    ) -> ViewConfig {
        
        var config = ViewConfig()
        
        // Check if the date is a weekend (Saturday or Sunday)
        if calendar.isDateInWeekend(state.date) {
            config.dateLabelColor = weekendColor // Set the desired color for weekend days
        } else {
            config.dateLabelColor = dateLabelColor // Default color for other days
        }
    ...
    struct ViewConfig {
        ...
        var dateLabelColor: UIColor = .label // Default color for date label text
        var weekendColor: UIColor = .red
    }
    ...
    internal func configure(for config: ViewConfig) {
        ...
        if let dateLabelText = config.dateLabelText {
            ...
            // Apply the specified date label color from the config
            self.dateLabel.textColor = config.dateLabelColor
